...
// Vista principal del tablero. Se inspira en el diseño visual del prototipo HTML.
// Sólo renderiza: el estado viene del gameStore.
// =============================================================================
...
import { CardFace, CardBackComponent, CardView } from '@/src/shared/ui/Card';
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <Pill dot>EN VIVO · MESA #{roomId}</Pill>
...
function ActivityFeed({ items }: { items: import('@/src/shared/types/game').ActivityEvent[] }) {
  return (
    <div style={{
      position: 'absolute', left: 12, bottom: 200, zIndex: 50,
      width: 200,
    }}>
      ...
      {items.slice(-5).reverse().map((item, i) => (
        <div key={item.id} style={{ marginBottom: 4, opacity: 1 - i * 0.18 }}>
          ...
          <span style={{ color: 'rgba(246,239,222,0.6)', fontSize: 11 }}>
            {' '}
            {item.kind === 'play' ? `tiró ${item.cardRanks?.join(', ')}` :
             item.kind === 'take_pile' ? 'recogió el pozo 💀' :
             item.kind === 'burn' ? '¡QUEMA! 🔥' :
             item.kind === 'intercept' ? `robó el turno a ${item.targetName}` :
             item.kind === 'intercept_fail' ? 'intercepción fallida' :
...
function OpponentCorner({ player, isActive, position }: {
  ...
  return (
    <div style={{ position: 'absolute', ...posStyle[position], zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
      {/* Avatar */}
      <div style={{
        ...
        boxShadow: isActive ? `0 0 20px ${player.avatarColor}` : '0 2px 8px rgba(0,0,0,0.5)',
        border: isActive ? `3px solid ${player.avatarColor}` : '2px solid rgba(255,255,255,0.1)',
      ...
      {/* Nombre + conteo */}
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontFamily: 'Anton, sans-serif', fontSize: 12, color: '#f6efde', letterSpacing: 1 }}>{player.name}</div>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'rgba(246,239,222,0.5)' }}>
          ✋ {player.handCount} cartas
        </div>
      </div>

      {/* Cartas de mesa (boca arriba) */}
      ...
      {player.tableDownCount > 0 && (
        <div style={{ display: 'flex', gap: 2 }}>
          {player.tableDown.map(cb => (
            <CardBackComponent key={cb.id} card={cb} size="small" />
          ))}
        </div>
      )}
      ...
      {isActive && (
        <div style={{
          ...
          background: '#FF6A1A', color: '#0e0b08',
...
function CenterTable({ topCard, deckCount, pileCount }: {
  topCard: Card | null;
  deckCount: number;
  pileCount: number;
}) {
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ position: 'relative', width: 88, height: 130 }}>
          <div style={{ position: 'absolute', top: 4, left: 4 }}><CardBackComponent card={{ id: 'back3', faceDown: true }} size="hand" /></div>
          <div style={{ position: 'absolute', top: 2, left: 2 }}><CardBackComponent card={{ id: 'back2', faceDown: true }} size="hand" /></div>
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{
          ...
          {topCard
            ? <CardFace card={topCard} size="pile" glow />
            : <span style={{ color: 'rgba(246,239,222,0.2)', fontSize: 28 }}>◌</span>
          }
        </div>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'rgba(246,239,222,0.5)', marginTop: 8, letterSpacing: 1 }}>
          {pileCount} EN MESA
...
function SelfHand({ self, selectedIds, onToggle, isMyTurn, onPlay, onTake, onStart, phase }: {
  ...
  return (
    <div style={{
      ...
      {/* Mesa boca arriba (si no hay mano) */}
      {isPlayingFromTableUp && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {self.tableUp.map(c => (
            <CardFace
              key={c.id} card={c} size="hand"
      ...
      {/* Ciegas (si no hay mano ni boca arriba) */}
      {isPlayingBlind && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {self.tableDown.map(cb => (
            <CardBackComponent
              key={cb.id} card={cb} size="hand"
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontFamily: 'Anton, sans-serif', fontSize: 40, color: '#FF2244', letterSpacing: 4 }}>💀 SHITHEAD</div>
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 14, color: 'rgba(246,239,222,0.7)', marginTop: 8 }}>
          {gameEnd.loserName} es el shithead de la noche
        </div>
        <div style={{ marginTop: 32 }}>
          {gameEnd.finishOrder.map(p => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 32, padding: '6px 0',
              fontFamily: 'JetBrains Mono, monospace', fontSize: 13,
              color: p.id === gameEnd.loserId ? '#FF2244' : '#f6efde',
            }}>
              <span>#{p.rank}</span>
              <span>{p.name}</span>
            </div>
...
export function GameBoard() {
  ...
  const { self, opponents, phase, discardTopCard, discardPileCount, deckCount,
          lastActivity, round, roomId, intercept } = gameView;

  const handlePlay = () => {
    if (self && self.hand.length === 0 && self.tableUp.length === 0 && self.tableDownCount > 0) {
      // Fase de ciegas — voltear la primera
