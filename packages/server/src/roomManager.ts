...
export function startGame(roomId: string): { ok: true; room: Room } | { ok: false; error: string } {
  ...
  if (gameState.players.length < 2) {
    return { ok: false, error: 'Se necesitan al menos 2 jugadores para iniciar' };
  }
...
export function handleDisconnect(playerId: string): { roomId: string | null; room: Room | null } {
...
export function buildRoomViews(room: Room): Map<string, ReturnType<typeof buildGameStateView>> {
  const views = new Map<string, ReturnType<typeof buildGameStateView>>();
  for (const player of room.gameState.players) {
    views.set(player.id, buildGameStateView(room.gameState, player.id, room.id));
  }
  return views;
}
